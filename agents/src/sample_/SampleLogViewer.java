...
import rescuecore2.standard.view.StandardEntityInspector;
...
public class SampleLogViewer extends JPanel {
    ...
    */
    public SampleLogViewer(LogReader reader, Config config) throws LogException {
        super(new BorderLayout());
        this.log = reader;
        inspector = new StandardEntityInspector();
                    ...
                    public void objectsClicked(ViewComponent view, List<RenderedObject> objects) {
                    	for (RenderedObject next : objects) {
                            if (next.getObject() instanceof Entity) {
                            	selectedEntityId=((Entity)next.getObject()).getID();
    ...
    public void showTimestep(int time) {
        try {
            ...
            if (updatesRecord != null) {
            	for (EntityID eid : updatesRecord.getChangeSet().getChangedEntities()) {
            		updates.add(model.getEntity(eid));
				}
            }
            inspector.inspect(model.getEntity(selectedEntityId));
            for (ViewComponent next : viewers) {
    ...
    */
    public static void main(String[] args) {
        ...
        try {
            ...
            SampleLogViewer viewer = new SampleLogViewer(reader, config);
